...
if path.exists('dopefactory_list.dat'):
    filehandler = open('dopefactory_list.dat', 'rb')
...
def SendDiscordMessage(itemName, itemColor, price, availability, URL, imageURL, sizes, webhook):
    Desc = '**Shop:** [Dope Factory (US)](https://www.dope-factory.com/collections/sneakers/)\n**Price:** ' + price + '\n**State:** ' + availability + '\n\n> QuickTask\n> [' + sizes + '](' + URL + ')';
    Color = 12726577
    Footer = '[SneakerAlpha]'
...
while True:
    time.sleep(10)
    pageNum += 1
    request = requests.get('https://www.dope-factory.com/collections/sneakers/products.json?limit=250&page=' + str(pageNum))
    ...
    for product in decodedJson['products']:
        ...
        ItemLink = 'https://www.dope-factory.com/products/' + product['handle']
    ...
    file = open('dopefactory_list.dat', 'wb')
