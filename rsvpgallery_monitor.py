...
if path.exists('rsvpgallery_list.dat'):
    filehandler = open('rsvpgallery_list.dat', 'rb')
...
def SendDiscordMessage(itemName, itemColor, price, availability, URL, imageURL, sizes, webhook):
    Desc = '**Shop:** [RSVP Gallery (US)](https://rsvpgallery.com/collections/footwear/)\n**Price:** ' + price + '\n**State:** ' + availability + '\n\n> QuickTask\n> [' + sizes + '](' + URL + ')';
    Color = 12726577
    Footer = '[SneakerAlpha]'
...
while True:
    time.sleep(10)
    pageNum += 1
    request = requests.get('https://rsvpgallery.com/collections/footwear/products.json?limit=250&page=' + str(pageNum))
    ...
    for product in decodedJson['products']:
        ...
        ItemLink = 'https://rsvpgallery.com/collections/footwear/products/' + product['handle']
    ...
    file = open('rsvpgallery_list.dat', 'wb')
