...
if path.exists('sneakerpolitics_list.dat'):
    filehandler = open('sneakerpolitics_list.dat', 'rb')
...
def SendDiscordMessage(itemName, itemColor, price, availability, URL, imageURL, sizes, webhook):
    Desc = '**Shop:** [Sneaker Politics (US)](https://sneakerpolitics.com/)\n**Price:** ' + price + '\n**State:** ' + availability + '\n\n> QuickTask\n> [' + sizes + '](' + URL + ')';
    Color = 12726577
    Footer = '[SneakerAlpha]'
...
while True:
    time.sleep(10)
    pageNum += 1
    request = requests.get('https://sneakerpolitics.com/products.json?limit=250&page=' + str(pageNum))
    ...
    for product in decodedJson['products']:
        ...
        ItemLink = 'https://sneakerpolitics.com/products/' + product['handle']
    ...
    file = open('sneakerpolitics_list.dat', 'wb')
