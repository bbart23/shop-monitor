...
if path.exists('travisscott_list.dat'):
    filehandler = open('travisscott_list.dat', 'rb')
...
def SendDiscordMessage(itemName, itemColor, price, availability, URL, imageURL, sizes, webhook):
    Desc = '**Shop:** [Travis Scott (US)](https://shop.travisscott.com/)\n**Price:** ' + price + '\n**State:** ' + availability + '\n\n> QuickTask\n> [' + sizes + '](' + URL + ')';
    Color = 12726577
    Footer = '[SneakerAlpha]'
...
while True:
    time.sleep(10)
    pageNum += 1
    request = requests.get('https://shop.travisscott.com/products.json?limit=250&page=' + str(pageNum))
    ...
    for product in decodedJson['products']:
        ...
        ItemLink = 'https://shop.travisscott.com/products/' + product['handle']
    ...
    file = open('travisscott_list.dat', 'wb')
