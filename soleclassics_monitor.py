...
if path.exists('soleclassics_list.dat'):
    filehandler = open('soleclassics_list.dat', 'rb')
...
def SendDiscordMessage(itemName, itemColor, price, availability, URL, imageURL, sizes, webhook):
    Desc = '**Shop:** [Sole Classics (US)](https://soleclassics.com/collections/shoes/)\n**Price:** ' + price + '\n**State:** ' + availability + '\n\n> QuickTask\n> [' + sizes + '](' + URL + ')';
    Color = 12726577
    Footer = '[SneakerAlpha]'
...
while True:
    time.sleep(10)
    pageNum += 1
    request = requests.get('https://soleclassics.com/collections/shoes/products.json?limit=250&page=' + str(pageNum))
    ...
    for product in decodedJson['products']:
        ...
        ItemLink = 'https://soleclassics.com/collections/shoes/products/' + product['handle']
    ...
    file = open('soleclassics_list.dat', 'wb')
