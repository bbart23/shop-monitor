...
if path.exists('saintalfred_list.dat'):
    filehandler = open('saintalfred_list.dat', 'rb')
    ItemList = pickle.load(filehandler)
else:
    ItemList = []

closetHook = 'https://discordapp.com/api/webhooks/739909910577414247/OGqr4QpMKSQqfXITpJ6MP76XN8Q-PNW_Jyr1clai_kjfJpfOGW3MWN92kDdCFzskzqox'
...
def SendDiscordMessage(itemName, itemColor, price, availability, URL, imageURL, sizes, webhook):
    Desc = '**Shop:** [Saint Alfred (US)](https://www.saintalfred.com/collections/footwear)\n**Price:** ' + price + '\n**State:** ' + availability + '\n\n> QuickTask\n> [' + sizes + '](' + URL + ')';
    Color = 12726577
    Footer = '[SneakerAlpha]'
...
while True:
    time.sleep(10)
    pageNum += 1
    request = requests.get('https://www.saintalfred.com/collections/footwear/products.json?limit=250&page=' + str(pageNum))
    ...
    for product in decodedJson['products']:
        ...
        ItemLink = 'https://www.saintalfred.com/collections/footwear/products/' + product['handle']
    ...
    file = open('saintalfred_list.dat', 'wb')
