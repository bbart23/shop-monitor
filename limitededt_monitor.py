...
if path.exists('limitededt_list.dat'):
    filehandler = open('limitededt_list.dat', 'rb')
    ItemList = pickle.load(filehandler)
else:
    ItemList = []

soleclassicsHook = 'https://discordapp.com/api/webhooks/739179663645212704/1WjlOarQ9hivXVZBE06Ff1E4nrFJKfr-BuYuoI00wlLncavLYa6djeO42mFhshqYbjFX'
...
def SendDiscordMessage(itemName, itemColor, price, availability, URL, imageURL, sizes, webhook):
    Desc = '**Shop:** [Limited Edt (US)](https://limitededt.com/collections/mens)\n**Price:** ' + price + '\n**State:** ' + availability + '\n\n> QuickTask\n> [' + sizes + '](' + URL + ')';
    Color = 12726577
    Footer = '[SneakerAlpha]'
...
while True:
    time.sleep(10)
    pageNum += 1
    request = requests.get('https://limitededt.com/collections/mens/products.json?limit=250&page=' + str(pageNum))
    ...
    for product in decodedJson['products']:
        ...
        ItemLink = 'https://limitededt.com/collections/mens/products/' + product['handle']
    ...
    file = open('limitededt_list.dat', 'wb')
